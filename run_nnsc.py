...
This is the main code to run NNSC from the input specifications using
...
from nnsc import names, param, plotting, draw
...
if params['version'] == 'deep_reg':#Regression with Leaky ReLU as activation functions
	mdict = deep_reg.main_reg(Xtrain, Ttrain.T, dim = dim, ct = ct, n_epochs = params['n_epochs'], batch_size = params['batch_size'], starter_learning_rate = params['learning_rate'], decay_after = params['lr_decay'], Ytest = Xtest, Ttest = Ttest.T)
elif params['version'] == 'deep_reg_est':#Same as deep_reg but Ttrain, Ttest values centred to 1
	mdict = deep_reg.main_reg(Xtrain, Ttrain.T + 1, dim = dim, ct = ct, n_epochs = params['n_epochs'], batch_size = params['batch_size'], starter_learning_rate = params['learning_rate'], decay_after = params['lr_decay'], Ytest = Xtest, Ttest = Ttest.T + 1)
    ...
    mdict = deep_reg.main_reg(Xtrain, Ttrain.T + 1, dim = dim, ct = ct, n_epochs = params['n_epochs'], batch_size = params['batch_size'], starter_learning_rate = params['learning_rate'], decay_after = params['lr_decay'], activation = activation, Ytest = Xtest, Ttest = Ttest.T + 1)
elif params['version'] == 'deep_regh':#Same as deep_reg but tanh as activation functions
    activation = 'tanh'
    mdict = deep_reg.main_reg(Xtrain, Ttrain.T, dim = dim, ct = ct, n_epochs = params['n_epochs'], batch_size = params['batch_size'], starter_learning_rate = params['learning_rate'], decay_after = params['lr_decay'], activation = activation, Ytest = Xtest, Ttest = Ttest.T)
...
R0 = deep_reg.ParamEstModel(mdict,Xtrain=Xtrain,Xtest=Xtest,Theta_test=Ttest.T, version = params['version'], activation = activation)
Pest = R0["Pest"]
Ptest = Ttest.T
E = Pest - Ptest
...
if params['save']:
    sio.savemat(output_path + fname + '.mat',mdict)
